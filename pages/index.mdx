...
When we request these documents, you can upload the necessary documents via the “Documents” section.

![bordered](rapp-upload-file.gif)

Now, let's start our journey by exploring Amsterdam, and its neighbourhoods and reading about the housing process.
...
export default function Index({ children }) {
    ...
    // Manually setting headings as it throws error inside
    // Wrapper because hell knows what
    const headings = [
        {
            name: 'About Relocify',
            depth: 1,
    ...
    return <Wrapper headings={headings}>{children}</Wrapper>
}
