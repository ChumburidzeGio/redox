    ...
    <div className="grid align-start gap-10 my-10 grid-cols-1 sm:grid-cols-2 max-w-2xl mx-auto">
        {items.map((item) => (
            <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex border border-gray-300 shadow overflow-hidden flex-col rounded-md no-underline relative h-fit"
            >
                <div className="flex h-[100px] overflow-hidden justify-center px-6 border-b border-gray-200">
                    <img
                        ...
                        className="block max-h-[100px] w-auto m-auto"
                    />
                </div>
                <div className="px-5 mt-4">
                    ...
                    <div className="text-slate-800">
                        {item.pros.map((pro) => (
                            <ProOrCon key={pro} type="pro">
                                {pro}
                            </ProOrCon>
                        ))}
                        {item.cons.map((con) => (
                            <ProOrCon key={con} type="con">
                                {con}
                            </ProOrCon>
                        ))}
                    </div>
